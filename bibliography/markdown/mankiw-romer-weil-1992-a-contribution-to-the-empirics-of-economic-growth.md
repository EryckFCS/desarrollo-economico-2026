...
Source: Lecturas/Bibliografía/Mankiw, Romer & Weil (1992) - A Contribution to the Empirics of Economic Growth.pdf
---
...
population growth, it does not correctly predict the magnitudes. In 
...
converges to a steady-state value k* defined by sk *a = (n + g + 8)k *, 
or 
(5) 
k* = [s/(n + g + 5)]1I(1-a) 
...
estimates of equation (7) using ordinary least squares are potentially inconsistent. 
In this case, to obtain consistent estimates, one needs to find instrumental variables 
that are correlated with s and n, but uncorrelated with the country-specific shift in 
the production function e. Finding such instrumental variables is a formidable task, 
however.
...
on equation (7) a value of ao derived from data on factor shares and 
...
unexplained part-the 
Solow residual-which 
is usually attributed to technological 
...
THE EMPIRICS OF ECONOMIC GROWTH 
413 
government investment) in real GDP, and Y/L as real GDP in 1985 
...
constraint that the coefficients on ln(s) and ln(n + g + 8) are equal 
in magnitude and opposite in sign. We assume that g + 8 is 0.05; 
...
6. We chose this value of g + 8 to match the available data. In U. S. data the 
capital consumption allowance is about 10 percent of GNP, and the capital-output 
ratio is about three, which implies that 8 is about 0.03; Romer [1989a, p. 60] 
presents a calculation for a broader sample of countries and concludes that 8 is
...
theory of growth in general-it 
implies that theories based on 
easily observable variables may be able to account for most of the 
cross-country variation in real income 
it is not supportive of the 
textbook Solow model in particular. 
II. ADDING HUMAN-CAPITAL ACCUMULATION 
TO THE SOLOW MODEL 
...
physical) are constant. We discuss this possibility in Section III. 
...
investment 
and population growth. We then run regressions 
...
for growth in income. Azariadis and Drazen [1990] find that no country was able to 
grow quickly during the postwar period without a highly literate labor force. They 
interpret this as evidence that there is a threshold externality associated with 
human capital accumulation. Similarly, Rauch [1988] finds that among countries 
that had achieved 95 percent adult literacy in 1960, there was a strong tendency for 
income per capita to converge over the period 1950-1985. Romer [1989b] finds that 
literacy in 1960 helps explain subsequent investment and that, if one corrects for 
measurement 
error, literacy has no impact on growth beyond its effect on 
investment. There is also older work stressing the role of human capital in 
development; for example, see Krueger [1968] and Easterlin [1981].
...
minimum wage-roughly 
the return to labor without human 
capital-has 
averaged about 30 to 50 percent of the average wage 
in manufacturing. This fact suggests that 50 to 70 percent of total 
labor income represents the return to human capital, or that ,B is 
...
P = 
1/3, then the coefficient on ln(sk) would be 1. Because higher 
...
= 1/3, for example, the 
coefficient on ln(n + g + 8) would be -2. 
In this model high 
...
augmented model's reduced form, that is, equation (11), in which 
...
8. Kendrick [19761 calculates that for the United States in 1969 total gross 
...
accumulation is reduced by Shl(l 
- Sh) compared with the elasticity of true GDP 
...
example, if a = P = 1/3 and Sh = 0.1, then the elasticity will be 0.9 rather than 1.0. 
10. Even under the weaker assumption that ln(sh) is linear in In (SCHOOL), 
we can use the estimated coefficients on ln(sk) and In (n + g + 5) to infer values of a 
...
Note. Standard errors are in parentheses. The investment and population growth rates are averages for the 
period 1960-1985. 
(g + 8) is assumed to be 0.05. SCHOOL is the average percentage of the working-age 
...
that the coefficients on In (IIY), In (SCHOOL), and In (n + g + 8) 
sum to zero. The bottom half of Table II shows that, for all three 
...
,B are about one third and highly significant. The estimates for the 
...
the worrisome anomalies-the 
high coefficients on investment and 
on population growth in our Table I regressions-that 
arise when 
...
that ln(sh) is linear in In (SCHOOL), estimates of a and , can be inferred from the 
coefficients on ln(I/GDP) and ln(n + g + 5) in the unrestricted regression. When we 
do this, we obtain estimates of a and , little different from those reported in 
Table II.
...
422 
QUARTERLY JOURNAL OF ECONOMICS 
Advocates of endogenous-growth 
models present them as 
...
In neoclassical growth models with diminishing returns, such as Solow (1956), Cass 
(1965) and Koopmans (1965), a country's per capita growth rate tends to be 
...
(14) 
ln(y(t)) = (1 - e-At) ln(y*) + e-At ln(y(0)), 
...
population growth rates.12 Endogenous-growth 
models with a 
...
growth. With these modifications to the model of Section II, the production function 
is Y = AKaH1-a. In this form the model predicts that the ratio of physical to human 
capital, K/H, will converge to SkiSh, and that K, H, and Y will then all grow at rate 
A(Sk)a(Sh 
)'-a. 
The derivative of this "steady-state" growth rate with respect to Sk is 
then aA(ShlSk) 
1- 
= atI(KIY). The impact of saving on growth depends on the
...
QUARTERLY JOURNAL OF ECONOMICS 
single sector-those 
with the "Y = AK" production function- 
...
regression in (16). As Barro [1989] notes, however, endogenous- 
...
those we presented earlier. The regressions in Tables I and II are 
...
have permanent differences in their production functions-that 
is, 
different A (O)'s-then these A(O)'s would enter as part of the error 
...
capital, a is close to one, and the derivative of the growth rate with respect to Sk is 
approximately 1 I (KI Y), or about 0.4. In models in which endogenous growth arises 
largely from human capital accumulation and there are no externalities from 
physical capital, the derivative would be about 0.31(KIY), or about 0.12.
...
gence. Moreover, the inclusion of investment 
and population 
...
restriction that the coefficients on ln(sk), ln(sh), and ln(n + g + 5) 
...
Table VI present the implied values of a and ,. The estimates of a 
range from 0.38 to 0.48, and the estimates of ,B are 0.23 in all three 
samples. Compared with the results in Table II, these regressions 
...
convergence. We find this interpretation plausible: World War II 
...
a value of X of 0.02, almost half of the departure from steady state 
...
430 
QUARTERLY JOURNAL OF ECONOMICS 
IV. INTEREST RATE DIFFERENTIALS AND CAPITAL MOVEMENTS 
Recently, several economists, including Lucas [1988], Barro 
[1989], and King and Rebelo [1989], have emphasized an objection 
...
be large. For example, if at = 1/3, 8 = 0.03, and g = 0.02, then the 
...
capital. Second, as Feldstein and Horioka [1980] first documented, 
...
which applies even outside of the steady state but requires an estimate of I and the 
...
then one can show that the MPK is proportional toy(a+P-1)/(a+P). Therefore, for the 
textbook Solow model in which a = 1/3 and I = 0, the MPK is inversely proportional 
to the square of output. As King and Rebelo [1989] and others have noted, the 
implied differences in rates of return across countries are incredibly large. Yet if a = 
0 = 
1/3, then the MPK is inversely proportional to the square root of output. In this 
...
capital's share. Sachs [1979] reports that Japan's rapid accumulation in the 1960s 
and 1970s, for example, was associated with a rise in labor's share from 69 percent 
in 1962-1964 to 77 percent in 1975-1978. See also Atkinson [1975, p. 167].
...
Psacharopoulos [1985] summarizes the results of studies for over 
...
THE EMPIRICS OF ECONOMIC GROWTH 
433 
unchanged. Higher saving thus raises total factor productivity as it 
...
elasticity of income per capita with respect to n + g + 8 is - '/2. In 
...
endogenous-growth 
models, this model predicts that countries 
...
BROWN UNIVERSITY 
REFERENCES 
Atkinson, Anthony, The Economics of Inequality (Oxford: Clarendon Press, 1975). 
Azariadis, Costas, and Allan Drazen, "Threshold Externalities 
in Economic 
Development," Quarterly Journal of Economics, CV (1990), 501-26. 
...
1950-85: Catch-Up and Convergence," American Economic Review, LXXIX 
(1989),1010-30. 
...
King, Robert G., and Sergio T. Rebelo, "Transitional Dynamics and Economic 
Growth in the Neoclassical Model," NBER Working Paper 3185, November 
1989. 
Krueger, Anne O., "Factor Endowments and per Capita Income Differences Among 
...
and Implications," Journal of Human Resources, XX (1985), 583-604. 
Rauch, James E., "The Question of International Convergence of per Capita 
Consumption: An Euler Equation Approach," mimeo, August 1988, University 
...
Business Cycle Theory, Robert J. Barro, ed. (Cambridge, MA: Harvard Univer- 
sity Press, 1989a), pp. 51-127. 
"Human Capital and Growth: Theory and Evidence," NBER Working Paper 
3173, November 1989b. 
Sachs, Jeffrey D., "Wages, Profit, and Macroeconomic Adjustment: A Comparative 
Study," Brookings Papers on Economic Activity (1979:2), 269-332. 
...
Real Product and Price Levels Estimates for 130 Countries, 1950-85," Review 
