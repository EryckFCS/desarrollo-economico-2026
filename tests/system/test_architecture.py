...
def test_vault_naming_convention():
    ...
    for item in vaults_path.iterdir():
        if item.is_dir():
            ...
            assert pattern.match(item.name), f"Vault '{item.name}' does not follow convention [unit]-[cat]-[seq]-[slug]."
    ...
    for vault in vaults_path.iterdir():
        if vault.is_dir():
            for req in required_internal:
                assert (vault / req).exists(), f"Vault '{vault.name}' is missing '{req}' for autonomy."
