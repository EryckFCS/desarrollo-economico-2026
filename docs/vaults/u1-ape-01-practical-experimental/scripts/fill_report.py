...
def fill_report():
    ...
    for p in doc.paragraphs:
        replace_text_in_paragraph(
            p, "(Nombre de la teoría asignada)", "Teoría Neomarxista de la Dependencia"
        )
        replace_text_in_paragraph(p, "Nombre:", "Nombre: Grupo 3 (Séptimo A)")
        replace_text_in_paragraph(p, "Fecha:", "Fecha: 04 de mayo de 2026")
        replace_text_in_paragraph(
            p, "(Proponer un título creativo)", "Crudo Castigo: La Anatomía Política del Valor"
        )
        replace_text_in_paragraph(
            p,
            "(Describir brevemente dónde y cuándo se desarrolla la historia)",
            "Venezuela, 1943. Un tribunal transhistórico audita la estructura de dependencia.",
        )
    ...
    for p in doc.paragraphs:
        ...
        if "Característica 1:" in p.text:
            p.text = ""
            p.add_run(
                "- Desarrollo del Subdesarrollo: André Gunder Frank (1967) postula que el subdesarrollo fue creado por el mismo proceso histórico que generó el desarrollo del centro."
            )
            p.paragraph_format.first_line_indent = Cm(1.27)
        if "Característica 2:" in p.text:
            p.text = ""
            p.add_run(
                "- Intercambio Desigual: Samir Amin (1976) explica la transferencia neta de valor desde la periferia hacia el centro debido a asimetrías tecnológicas."
            )
            p.paragraph_format.first_line_indent = Cm(1.27)
        if "Característica 3:" in p.text:
            p.text = ""
            p.add_run(
                "- Alianzas de Clase: La dependencia se reproduce mediante alianzas entre el capital transnacional y las élites locales (lumpenburguesía)."
            )
            p.paragraph_format.first_line_indent = Cm(1.27)
        ...
        if "Política 1:" in p.text:
            p.text = ""
            p.add_run(
                "- La Desconexión (Delinking): Subordinar las relaciones externas a las necesidades del desarrollo interno (Amin, 1976)."
            )
            p.paragraph_format.first_line_indent = Cm(1.27)
        if "Política 2:" in p.text:
            p.text = ""
            p.add_run(
                "- Nacionalización Estratégica: Control estatal de sectores clave para retener el excedente económico."
            )
            p.paragraph_format.first_line_indent = Cm(1.27)
        if "Política 3:" in p.text:
            p.text = ""
            p.add_run(
                "- Cooperación Sur-Sur: Alianzas con otros países periféricos para romper el monopolio tecnológico."
            )
            p.paragraph_format.first_line_indent = Cm(1.27)
    ...
    if target_idx != -1:
        ...
        for j in range(target_idx + 1, target_idx + 6):
            if j < len(doc.paragraphs) and (
                "(Nombre del personaje" in doc.paragraphs[j].text
                or doc.paragraphs[j].text.strip() == "-"
            ):
