...
capture confirm file "data/personas_2025_anual.dta"
if _rc != 0 {
    * Si no encuentra los datos en el directorio actual, sube un nivel (asumiendo ejecución desde scripts/)
    cd ".."
}
global ROOT "."

* Indique si desea usar el archivo original SPSS (.sav) o el archivo Stata (.dta)
global USAR_SPSS 0  // Cambie a 1 si desea importar desde SPSS (.sav)

* Configurar registro de ejecución (Log) y desactivar gráficos en pantalla
capture log close
log using "$ROOT/logs/ejecucion_codigo_grupal.log", replace text
set graphics off

if $USAR_SPSS == 1 {
    import spss using "$ROOT/data/BDDenemdu_personas_2025_anual.sav"
}
else {
    use "$ROOT/data/personas_2025_anual.dta", clear
}
...
* [clean-opt] tab ciudad

///////////////  1.2 CONSERVAR VARIABLES IMPORTANTES

keep ///
prov ciudad area dominio periodo mes /// *Identificacion territorial*
...
keep ///
prov ciudad area dominio periodo mes ///
...
save "$ROOT/data/basepichincha.dta", replace
use "$ROOT/data/basepichincha.dta", clear
...
global G_II = (ln(ING)-ln(100))/(ln(75000)-ln(100))
scalar II = $G_II
...
* Guardar la base que ya contiene la variable esc para uso posterior
save "$ROOT/data/basepichincha.dta", replace
...
* cd "$ROOT/data" // Cambiado para portabilidad
* Revisar la variable de pobreza por ingresos
* [clean-opt] describe pobreza
* [clean-opt] tab pobreza, missing

* Verificar categorías
capture label list labels109
...
* [clean-opt] tab pobreza_ingresos, missing
...
* [clean-opt] tab educ_inasistencia if inrange(p03,5,17), missing

* Etiqueta
label define privacion 0 "No privado" 1 "Privado"
label values educ_inasistencia privacion

* [clean-opt] tab educ_inasistencia, missing
* [clean-opt] tab p07 educ_inasistencia if inrange(p03,5,17), row
...
* [clean-opt] tab d2_superior if inrange(p03,18,29), missing
...
* [clean-opt] tab educ_logro, missing             
* [clean-opt] tab p10a educ_logro if p03>=18, row
...
* [clean-opt] tab dim_educacion, missing
* [clean-opt] tab priv_educacion dim_educacion, missing

save "$ROOT/data/base_IPM_educacion.dta", replace

* Generar base a nivel de hogar para el merge posterior
preserve
    keep id_hogar priv_educacion
    duplicates drop id_hogar, force
    save "$ROOT/data/base_IPM_educacion_hogar.dta", replace
restore
...
* [clean-opt] tab pensiones, missing
* [clean-opt] tab p61b1 pensiones if p03>=15 & empleo==1, row
...
* [clean-opt] tab trabajo_infantil, missing
* [clean-opt] tab p20 trabajo_infantil if inrange(p03,5,17), row
...
* [clean-opt] tab trabajo_inadecuado, missing
* [clean-opt] tab condact trabajo_inadecuado if p03>=15, row
...
* [clean-opt] tab h_pensiones, missing
* [clean-opt] tab h_trabajo_infantil, missing
* [clean-opt] tab h_empleo_inadecuado, missing
...
* [clean-opt] tab dim_trabajo, missing
* [clean-opt] tab priv_trabajo, missing
sum dim_trabajo


save "$ROOT/data/base_IPM_trabajo.dta", replace

* Generar base a nivel de hogar para el merge posterior
preserve
    keep id_hogar priv_trabajo
    duplicates drop id_hogar, force
    save "$ROOT/data/base_IPM_trabajo_hogar.dta", replace
restore



clear
use "$ROOT/data/vivienda_2025_anual.dta", clear
keep if prov == "17"
...
save "$ROOT/data/agua_hogar.dta", replace
...
* [clean-opt] tab pobreza_extrema, missing
...
merge m:1 id_hogar using "$ROOT/data/agua_hogar.dta"
drop _merge
*-------------------------------------------------------------------------------
...
* [clean-opt] tab punt_salud_agua, missing

save "$ROOT/data/base_IPM_salud.dta", replace
...
* [clean-opt] tab priv_salud_agua, missing
...
* (Se asume que en memoria tenemos la base personas completa, pero se borró tras cargar la base de salud. 
* Espera, en la línea 595 se hizo use basepichincha, y luego en la 632 se hizo keep id_hogar etc, 
* por lo que en memoria quedan solo esas variables. 
* Para calcular tam_hogar de forma correcta sobre la base de personas, necesitamos volver a cargar basepichincha).
* Agreguemos: use "$ROOT/data/basepichincha.dta", clear
use "$ROOT/data/basepichincha.dta", clear
bysort id_hogar: gen tam_hogar = _N
* [clean-opt] tab tam_hogar
...
save "$ROOT/data/tam_hogar.dta", replace

* 4. Unir tamaño del hogar con vivienda
use "$ROOT/data/vivienda_2025_anual.dta", clear
keep if prov == "17"

* [clean-opt] describe id_hogar vi07
merge 1:1 id_hogar using "$ROOT/data/tam_hogar.dta"
* [clean-opt] tab _merge
...
* [clean-opt] tab hacinamiento, missing
...
* [clean-opt] tab deficit_habitacional, missing
...
* [clean-opt] tab saneamiento, missing
* [clean-opt] tab vi09 saneamiento, row
...
* [clean-opt] tab basura, missing

save "$ROOT/data/ENEMDU_Vivienda_Pichincha_indicadores.dta", replace

*-------------------------------------------------------------------------------
use "$ROOT/data/ENEMDU_Vivienda_Pichincha_indicadores.dta", clear
keep id_hogar hacinamiento deficit_habitacional saneamiento basura
describe
save "$ROOT/data/indicadores_vivienda_pichincha.dta", replace
*-------------------------------------------------------------------------------
use "$ROOT/data/basepichincha.dta", clear
merge m:1 id_hogar using "$ROOT/data/indicadores_vivienda_pichincha.dta"
*-------------------------------------------------------------------------------
...
* [clean-opt] tab priv_habitat

save "$ROOT/data/base_personas_pichincha_dimension4.dta", replace
...
save "$ROOT/data/base_IPM_habitat.dta", replace

clear

use "$ROOT/data/base_IPM_educacion_hogar.dta", clear

merge 1:1 id_hogar using "$ROOT/data/base_IPM_trabajo_hogar.dta"
* [clean-opt] tab _merge
drop _merge

merge 1:1 id_hogar using "$ROOT/data/base_IPM_salud.dta"
* [clean-opt] tab _merge
drop _merge

merge 1:1 id_hogar using "$ROOT/data/base_IPM_habitat.dta"
* [clean-opt] tab _merge
drop _merge
save "$ROOT/data/base_IPM_final.dta", replace

 //CÁLCULO DEL IPM OFICIAL 
* cd "$ROOT/data"
* La base ya está cargada en memoria, no es necesario hacer clear y use de nuevo
* 1. Verificar variables de privación
* [clean-opt] describe priv_*

* [clean-opt] tab priv_educacion
* [clean-opt] tab priv_trabajo
* [clean-opt] tab priv_salud_agua
* [clean-opt] tab priv_habitat
...
* [clean-opt] tab priv_activos
* [clean-opt] summ punt_activos

keep id_hogar priv_activos
save "$ROOT/data/indicadores_activos_pichincha.dta", replace
...
* [clean-opt] tab sin_vehiculo, missing
*-------------------------------------------------------------------------------
...
* [clean-opt] tab sin_moto, missing
*------------------------------------------------------------------------------
...
* [clean-opt] tab priv_activos, missing
...
save "$ROOT/data/indicadores_activos_pichincha.dta", replace
*------------------------------------------------------
use "$ROOT/data/base_IPM_final.dta", clear

merge m:1 id_hogar using "$ROOT/data/indicadores_activos_pichincha.dta"
...
* [clean-opt] tab priv_activos
save "$ROOT/data/base_personas_pichincha_dim4_dim5.dta", replace


* CÁLCULO DEL ÍNDICE DE POBREZA MULTIDIMENSIONAL MODIFICADO

* cd "$ROOT/data"
* use "$ROOT/data/base_personas_pichincha_dim4_dim5.dta", clear (Redundante, ya en memoria)
...
* [clean-opt] tab punt_ipm_ampliado

* Identificación de pobreza multidimensional
* Umbral k = 1/3
gen pobre_multi_ampliado = 0
replace pobre_multi_ampliado = 1 if punt_ipm_ampliado >= 0.333333 & !missing(punt_ipm_ampliado)
* [clean-opt] tab pobre_multi_ampliado
...
* [clean-opt] tab privaciones_ampliado
...
* [clean-opt] list area prom_area theil_area theil_dentro_area theil_entre_area if tag_area, noobs
...
* [clean-opt] list area_nombre  theil_area if tag_area, noobs
...
* Ver tabla
* [clean-opt] list decil poblacion_decil ingreso_decil in 1/11, noobs
...
graph export "$ROOT/assets/curva_lorenz_2025.png", replace
...
* [clean-opt] list decil_nac part_ing_nac x_nac y_nac, noobs
...
graph export "$ROOT/assets/distribucion_ingreso_deciles.png", replace
...
* [clean-opt] list area_nombregini gini_area if tag_areagini, noobs
...
* Restaurar escalares del IDH desde variables globales
scalar IE = $G_IE
scalar IS = $G_IS
scalar II = $G_II
...
* Cerrar registro de ejecución (Log)
log close