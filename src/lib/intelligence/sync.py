...
class IntelligenceOrchestrator:
    ...
    def sync_rag(self):
        ...
        logger.info("Sincronizando directorio sanitizado con el RAG.")

        try:
            # MarkdownRAG.index_directory ya maneja la iteración y logging
            result = self.rag.index_directory(
                corpus_dir=self.sanitized_dir, exclude={"conversion_report.json"}
            )
            logger.info(
                f"RAG actualizado: {result['files_processed']} archivos indexados ({result['total_chunks']} chunks)."
            )
            if result["errors"]:
